...
| <b>rectRvalue</b>([<i>number</i>]) | Getter/setter for the timeline `rect` corner rounding `rx/ry` parameters | 1 |
...
| <b>zColorScale</b>([<i>d3 scale object</i>]) | Getter/setter for the color scale to be used for coloring the segments according to their data values. This object should be a D3 color scale object. <br>Example for qualitative:<br> `import {scaleOrdinal} from "d3-scale";` <br>Then chain with `.zColorScale(scaleOrdinal(['rgb(37, 99, 235)', '#ff0000', '#00ffff']))`| qualitative: `d3.scaleOrdinal(<color-list>)` <br> quantitative: `d3.scaleSequential(<color-interpolator>)` |
